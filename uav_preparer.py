...
class UAVPreparer:
    ...
    def help(self):
        url = "https://github.com/CasRenetteGU/UAVPreparer"
        webbrowser.open_new_tab(url)

    def start_progress(self):
        if not self.outputfile:
            QMessageBox.critical(None, "Error", "SPecify output file")
            return
        
        dsm_layer = self.layerComboManagerDSM.currentLayer()
        if dsm_layer is None:
            QMessageBox.critical(None, "Error", "No valid raster selected")
            return
        else:        
        ...
        if point_layer is None:
            QMessageBox.critical(None, "Error", "No valid vector point layer selected")
            return
        else:
            vlayer = QgsVectorLayer(point_layer.source(), "polygon", "org")

        point_field = self.layerComboManagerPointField.currentField()
        idx = vlayer.fields().indexFromName(point_field)
        if idx == 1:
            QMessageBox.critical(None, "Error", "Attribute with unique value fields must be selected")
            return
        ...
        for f in vlayer.getFeatures():
            ...
            result[i, 1] = mat.mean(mat)
            result[i, 2] = mat.max(mat)
            result[i, 3] = mat.min(mat)
            i += 1
        headetext = 'id mean max min'
        numformat = '%d %6.2f %6.2f %6.2f'
        np.savetxt(self.outputfile[0], result, fmt=numformat, delimiter=' ', header=headetext)
